...
def _source_is_dirty() -> bool:
    """Whether production source differs from the revision recorded in the manifest."""
    try:
...
def main() -> int:
    if _source_is_dirty():
        raise RuntimeError("production source is dirty; inputs must be generated by an unmodified revision")
