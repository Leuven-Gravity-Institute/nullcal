...
class MockSamplesSummary:
    def __init__(self, data, average='mean', confidence_level=0.9):
        ...
        self.lower_relative_credible_interval = self.average - \
            np.percentile(data, (1 - confidence_level) / 2 * 100, axis=0)
        self.upper_relative_credible_interval = np.percentile(
            data, (1 + confidence_level) / 2 * 100, axis=0) - self.average
        self.lower_absolute_credible_interval = np.percentile(
            data, (1 - confidence_level) / 2 * 100, axis=0)
        self.upper_absolute_credible_interval = np.percentile(
            data, (1 + confidence_level) / 2 * 100, axis=0)
...
def test_plot_spline_pos_basic(mock_interp1d, mock_legend, mock_xlim, mock_plot, mock_fill_between, mock_errorbar):
    ...
    with patch('__main__.SamplesSummary', MockSamplesSummary):
        # Call the function
        plot_spline_pos(log_freqs, samples, minimum_frequency, maximum_frequency, nfreqs, level,
                        injected_values=None, priors=None, errorbar=False, color=color, label=label, xform=None)
...
def test_plot_spline_pos_with_errorbar_and_xform(mock_interp1d, mock_plot, mock_fill_between, mock_errorbar):
    ...
    with patch('__main__.SamplesSummary', MockSamplesSummary):
        plot_spline_pos(log_freqs, samples, minimum_frequency, maximum_frequency, nfreqs, level,
                        injected_values=None, priors=None, errorbar=True, color=color, label=None, xform=xform)
...
def test_plot_spline_pos_with_injected_values(mock_interp1d, mock_plot):
    ...
    with patch('__main__.SamplesSummary', MockSamplesSummary):
        plot_spline_pos(log_freqs, samples, minimum_frequency, maximum_frequency, nfreqs, level,
                        injected_values=injected_values, priors=None, errorbar=False)
...
def test_plot_spline_pos_with_priors(mock_interp1d, mock_plot):
    ...
    with patch('__main__.SamplesSummary', MockSamplesSummary):
        plot_spline_pos(log_freqs, samples, minimum_frequency, maximum_frequency, nfreqs, level,
                        injected_values=None, priors=priors, errorbar=False)
...
def test_plot_calibration_posterior_no_ifos(mock_plot_spline_pos, mock_show, mock_tight_layout, mock_sca, mock_subplots):
...
@patch('matplotlib.pyplot.show')
@patch('plot_functions.plot_spline_pos')
def test_plot_calibration_posterior_with_ifos(mock_plot_spline_pos, mock_show, mock_tight_layout, mock_sca, mock_subplots):
    # Mock posterior with calibration parameters
    mock_posterior = MagicMock()
    mock_posterior.keys.return_value = [
        'recalib_ET1_frequency_0', 'recalib_ET1_amplitude_0', 'recalib_ET1_phase_0'
    ]
...
def test_plot_calibration_posterior_save_file(mock_plot_spline_pos, mock_save_figure, mock_sca, mock_subplots):
    ...
    plot_calibration_posterior(result, filename='test.png',
                               show_injected_value=True, show_priors=True)
