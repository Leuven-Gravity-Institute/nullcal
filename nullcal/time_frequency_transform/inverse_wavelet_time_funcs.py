...
def unpack_time_wave_helper(int n,
                                   int Nf,
                                   int Nt,
                                   int K,
...
def unpack_time_wave_helper(n,
                            ...
                            fft_fin_real,
                            res):
    """Unpack the wave in time domain.
    ...
    Args:
        ...
        fft_fin_real (1D numpy array): FFT
...
@njit
def unpack_time_wave_helper_compact(n,
                                    Nf,
                                    Nt,
                                    K,
                                    phis,
    ...
    Args:
        ...
        Nf (int): Number of frequency bins
        Nt (int): Number of time bins
        K (int): Frequency cut-off
        phis (1D numpy array): Wavelet
        fft_fin (1D numpy): FFT
        res (1D numpy array): Result
    """
    ND = Nf*Nt
    fft_fin_real = np.zeros(4*Nf,dtype=np.float64)
    fft_fin_imag = np.zeros(4*Nf,dtype=np.float64)
...
@njit
def pack_wave_time_helper_compact(n,
                                  Nf,
                                  Nt,
                                  wave_in,
                                  afins):
...
@njit
def inverse_wavelet_time_helper_fast(wave_in,
                                     phi,
                                     Nf,
                                     Nt,
                                     mult):
    ...
    res = np.zeros(ND+K+Nf, dtype=np.float64)
