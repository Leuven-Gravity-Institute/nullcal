"""
A Result class to process the analysis results.
"""
...
class Result(CoreResult):
    ...
    def detectors(self) -> np.ndarray:
        ...
        if self.search_parameter_keys is None:
            raise ValueError("self.search_parameter_keys is None.")
        if self._detectors is None:
            self._detectors = np.unique(
                [param.split("_")[1] for param in self.search_parameter_keys if "recalib_" in param]
            )
    ...
    def posterior_volume_from_samples(self, samples: np.ndarray) -> float:
        ...
        if covariance_matrix.ndim == 0:
            return np.sqrt(covariance_matrix)[0, 0]
    ...
    def get_calibration_frequency_parameters(self, detector: str) -> np.ndarray:
        ...
        """
        if self.fixed_parameter_keys is None:
            raise ValueError("self.fixed_parameter_keys is None.")
    ...
    def get_injection_calibration_amplitude_error(self, detector: str) -> np.ndarray:
        ...
        if self.injection_parameters is None:
            raise ValueError("self.injection_parameters is None.")
        injected_values = self.injection_parameters[0]
    ...
    def get_calibration_amplitude_error_priors(self, detector: str) -> np.ndarray:
        ...
        if isinstance(priors, dict):
            ...
            raise ValueError("priors are not found.")
        else:
    ...
    #     priors_amplitude = np.transpose([self.get_calibration_amplitude_error_priors(ifo) for ifo in self.detectors])
    #     priors_phase = np.transpose([self.get_calibration_phase_error_priors(ifo) for ifo in self.detectors])
    #     priors_at_knot_frequency = np.column_stack((priors_amplitude, priors_phase))

    #     # Get the posterior samples at knot frequencies
    #     posterior_amplitude = np.transpose(
    #         [self.get_calibration_amplitude_error_posterior_samples(ifo) for ifo in self.detectors], axes=(2, 0, 1)
    #     )
    #     posterior_phase = np.transpose(
    #         [self.get_calibration_phase_error_posterior_samples(ifo) for ifo in self.detectors], axes=(2, 0, 1)
    #     )
    ...
    #     priors_volume_at_knot_frequency = np.array(
    #         [self.gaussian_prior_volume(priors) for priors in priors_at_knot_frequency]
    #     )
    #     posterior_volume_at_knot_frequency = np.array(
    #         [self.posterior_volume_from_samples(posterior) for posterior in posterior_at_knot_frequency]
    #     )
    ...
    def mahalanobis_distance(self, samples, point) -> float:
        ...
        if len(point) != dimension:
            raise ValueError(
                "Impossible to compute the Mahalanobis distance between"
                f"{dimension}-D distribution and {len(point)}-D array"
            )
    ...
    def compute_calibration_amplitude_bias_at_knot_frequency(self, use_priors: bool | None = False) -> np.ndarray:
        ...
        N-dimensional calibration amplitude distributions (posterior or prior),
        evaluated at knot frequencies.

        Args:
            use_priors (bool, optional): If True, compute the distance
                using the prior distribution instead of the posterior. Defaults to False.
        ...
        injected_amplitude_at_knot_frequency = np.transpose(
            [self.get_injection_calibration_amplitude_error(ifo) for ifo in self.detectors]
        )

        distance = [
            self.mahalanobis_distance(samples, inj_value)
            for (samples, inj_value) in zip(
                samples_amplitude_at_knot_frequency, injected_amplitude_at_knot_frequency, strict=False
            )
        ]
    ...
    def compute_calibration_phase_bias_at_knot_frequency(self, use_priors: bool | None = False) -> np.ndarray:
        ...
        Args:
            use_priors (bool, optional): If True, compute the distance using the prior distribution
                instead of the posterior. Defaults to False.
        ...
        # Get the injected values at knot frequency
        injected_phase_at_knot_frequency = np.transpose(
            [self.get_injection_calibration_phase_error(ifo) for ifo in self.detectors]
        )

        distance = [
            self.mahalanobis_distance(samples, inj_value)
            for (samples, inj_value) in zip(
                samples_phase_at_knot_frequency, injected_phase_at_knot_frequency, strict=False
            )
        ]

        return np.array(distance)

    # def plot_calibration_posterior(
    #     self,
    ...
    #     show_error_bar: bool | None = False,
    ...
    #         show_error_bar (bool, optional): If true, display the posterior error bars at the spline control points
    ...
    #     log_freqs = np.log(self.get_calibration_knot_frequency_array())
    ...
    #             errorbar=show_error_bar,
    #             color=color,
    #             label=rf"{ifo.upper()} {int(quantile * 100)}$\%$C.L.",
    ...
    # def plot_relative_calibration_posterior(
    #     self,
    #     filename: str | None = None,
    #     minimum_frequency: float | None = None,
    #     maximum_frequency: float | None = None,
    ...
    #             errorbar=show_error_bar,
    #             color=color,
    #             label=rf"{ifo.upper()}/{self.detectors[0]} {int(quantile * 100)}$\%$C.L.",
    #         )
