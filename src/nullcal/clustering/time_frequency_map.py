...
def construct_time_frequency_map(interferometers, frequency_resolution, nx, *arg, **kwargs):
    ...
    whitened_time_frequency_domain_strain_0 = transform_wavelet_freq(whitened_frequency_domain_strain[0], Nf, Nt, nx)
    whitened_time_frequency_domain_strain_quadrature_0 = transform_wavelet_freq_quadrature(
        whitened_frequency_domain_strain[0], Nf, Nt, nx
    )
    combined_power = (
        whitened_time_frequency_domain_strain_0**2 + whitened_time_frequency_domain_strain_quadrature_0**2
    ) / 2
    for i in range(1, n_det):
        whitened_time_frequency_domain_strain_i = transform_wavelet_freq(
            whitened_frequency_domain_strain[i], Nf, Nt, nx
        )
        whitened_time_frequency_domain_strain_quadrature_i = transform_wavelet_freq_quadrature(
            whitened_frequency_domain_strain[i], Nf, Nt, nx
        )
        combined_power += (
            whitened_time_frequency_domain_strain_i**2 + whitened_time_frequency_domain_strain_quadrature_i**2
        ) / 2
