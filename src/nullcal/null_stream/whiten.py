...
def compute_whitened_frequency_domain_strain(
    ...
    if frequency_domain_strain_array.shape != power_spectral_density_array.shape:
        raise ValueError(
            f"Shape mismatch. frequency_domain_strain_array: {frequency_domain_strain_array.shape} and power_spectral_density_array: {power_spectral_density_array.shape} must have the same dimensions."
        )
    if frequency_domain_strain_array.shape[1] != frequency_mask.shape[0]:
        raise ValueError(
            f"Shape mismatch. frequency_domain_strain_array: {frequency_domain_strain_array.shape} and frequency_mask: {frequency_mask.shape} must have the same frequency dimension."
        )
    return np.divide(
...
def compute_whitened_antenna_response(
    ...
    if n_det_1 != n_det_2:
        raise ValueError(
            f"Shape mismatch. antenna_response_matrix: (detector={n_det_1}, mode={n_mode_1}) and power_spectral_density_array: (detector={n_det_2}, frequency={n_freq_2}) must have the same detector dimension."
        )
    if n_freq_2 != n_freq_3:
        raise ValueError(
            f"Shape mismatch. power_spectral_density_array: (detector={n_det_2}, freq={n_freq_2}) and frequency_mask: (frequency: {n_freq_3}) must have the same frequency dimension."
        )
