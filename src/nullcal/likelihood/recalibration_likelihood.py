...
class RecalibrationLikelihood(Likelihood):
    ...
    def interferometers(self, value: InterferometerList):
        ...
        if not np.allclose(sampling_frequency_array, sampling_frequency_array[0]):
            raise ValueError(f'The interferometers do not have the same sampling frequency: {sampling_frequency_array}.')
    ...
    def time_frequency_filter(self) -> np.ndarray | None:
        ...
        if self._time_frequency_filter is None and self.clustering_parameter_file is not None:
            ...
            self._time_frequency_filter = single_clustering_by_threshold(interferometers=interferometers,
    ...
    def compute_uncalibrated_frequency_domain_null_stream(self) -> np.ndarray:
        ...
        projector = compute_projector(self._whitened_antenna_response,
                                      frequency_mask=self.frequency_mask)
    ...
    def compute_calibrated_frequency_domain_null_stream(self,
                                                        calibration_factor: np.ndarray) -> np.ndarray:
    ...
    def construct_calibration_factor_from_parameters(self, parameters: dict) -> np.ndarray:
        ...
        calibration_factor =  np.array([ifo.calibration_model.get_calibration_factor(frequency_array=self.masked_frequency_array,
                                                                                     ...
                                                                                     **parameters) for ifo in self.interferometers])
    ...
    def log_likelihood(self) -> float:
        ...
        residual_energy = float(np.sum(np.abs(calibrated_time_frequency_domain_null_stream[:, self.time_frequency_filter])**2))
