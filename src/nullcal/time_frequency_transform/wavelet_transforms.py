...
def inverse_wavelet_time(wave_in, Nf, Nt, nx=4.0, mult=32):
...
def inverse_wavelet_freq_time(wave_in, Nf, Nt, nx=4.0):
...
def inverse_wavelet_freq(wave_in, Nf, Nt, nx=4.0):
...
def transform_wavelet_time(data, Nf, Nt, nx=4.0, mult=32):
...
def transform_wavelet_freq_time(data, Nf, Nt, nx=4.0):
...
def transform_wavelet_freq_time_quadrature(data, Nf, Nt, nx=4.0):
...
def transform_wavelet_freq_quadrature(data, Nf, Nt, nx=4.0):
...
def transform_wavelet_freq(data, Nf, Nt, nx=4.0):
...
def transform_wavelet_freq_partial(data, Nf, Nt, nx, frequency_filter):
    ...
    """
    phif = 2 / Nf * phitilde_vec_norm(Nf, Nt, nx)
    return transform_wavelet_freq_partial_helper(data, Nf, Nt, phif, frequency_filter) * np.sqrt(
        (data.shape[0] - 1) * 2
    )


def transform_wavelet_time_pixel(data, Nf, Nt, nx=4.0, mult=32, time_frequency_filter=None):
    """Do the wavelet transform in the time domain,
    note there can be significant leakage if mult is too small and the
    transform is only approximately exact if mult=Nt/2.
